    ...
    def run_tests(self, phase1_1_multiproc, phase1_2_collectives, phase1_3_devicemesh,
        ...
        try:
            ...
            if phase2_2_fsdp2_api:
                logging.info(f"{LOG_PREFIX} [Test] ┌─────────────────────────────────────────────────────────┐")
                logging.info(f"{LOG_PREFIX} [Test] │ PHASE 2.2: FSDP2ModelPatcher Functionality (COMPLETE)  │")
                logging.info(f"{LOG_PREFIX} [Test] └─────────────────────────────────────────────────────────┘")
                
                logging.info(f"{LOG_PREFIX} [Test] Test 2.2: FSDP2ModelPatcher functionality...")
                result = executor.execute_collective("test_fsdp2_modelpatcher", {})
                
                # Check all validations passed
                if not result.get("all_passed", False):
                    failed_checks = []
                    if not result.get("extends_modelpatcher", False):
                        failed_checks.append("Doesn't extend ModelPatcher")
                    if not result.get("has_original_size", False):
                        failed_checks.append("Missing original_model_size attribute")
                    if not result.get("correct_size", False):
                        failed_checks.append("Original size not tracked correctly")
                    if not result.get("correct_sharded_size", False):
                        failed_checks.append("Sharded size calculation wrong")
                    if not result.get("has_model_type", False):
                        failed_checks.append("Missing model_type attribute")
                    if not result.get("has_rank", False):
                        failed_checks.append("Missing/wrong rank attribute")
                    if not result.get("has_world_size", False):
                        failed_checks.append("Missing/wrong world_size attribute")
                    if not result.get("has_is_wrapped_flag", False):
                        failed_checks.append("Missing is_fsdp2_wrapped flag")
                    
                    if executor: executor.shutdown()
                    failure_msg = f"❌ FAIL [Test 2.2]: FSDP2ModelPatcher incomplete ({result.get('passed_checks', '0/8')})\n"
                    for check in failed_checks:
                        failure_msg += f"  ❌ {check}\n"
                    return (failure_msg,)
                
                logging.info(f"{LOG_PREFIX} [Test] ✅ PASS [Test 2.2]: FSDP2ModelPatcher functional")
                logging.info(f"{LOG_PREFIX} [Test]   Checks: {result.get('passed_checks', '0/8')}")
