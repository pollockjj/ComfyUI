...
class ParallelAttentionUnitTests:
    """Phase 1 Foundation Tests - DeviceMesh, Executor, Policies.
    
    Tests the salvaged core infrastructure:
    - FSDP2Executor (DeviceMesh-based)
    - parallel_state (DeviceMesh management)
    - fsdp2_policies (Policy registry)
    - fsdp2_engine (Structure application)
    
    Phase 2 will add LPS Iterator tests.
    """
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "test_engine_structure": ("BOOLEAN", {"default": False}),
                
                # Convenience
                "run_all_phase1": ("BOOLEAN", {"default": True}),
            },
            "optional": {
                "model": ("MODEL",),
            }
    ...
    def run_tests(self, test_executor_spawn, test_devicemesh_init,
                  ...
                  test_engine_structure, run_all_phase1, model=None):
        """Run Phase 1 foundation tests for parallel attention.
        
        Tests salvaged core infrastructure with DeviceMesh.
        """
        ...
        if run_all_complete:
            ...
            phase2_7_fsdp2_sharding = True
            phase2_7_2_deep_validation = True
            phase2_10_meta_shell = True
            phase2_10_pickling = True
            phase2_10_worker_init = True
        ...
        try:
            ...
            if model is not None and hasattr(model, 'executor') and model.executor is not None:
            ...
            if phase2_5_worker_spawn:
                ...
                if model is None:
                    ...
                    # Check 1: Has executor attribute
                    has_executor_attr = hasattr(model, 'executor') and model.executor is not None
                    logging.info(f"{LOG_PREFIX} [Test]   {'✅' if has_executor_attr else '❌'} [1/5] Has executor attribute")
            ...
            # ═══════════════════════════════════════════════════════════════
            # PHASE 2.6: Eager Initialization Validation (Test 2.6)
            # ═══════════════════════════════════════════════════════════════
            if phase2_6_checkpoint_path:
                logging.info(f"{LOG_PREFIX} [Test] ┌─────────────────────────────────────────────────────────┐")
                logging.info(f"{LOG_PREFIX} [Test] │ PHASE 2.6: Eager Initialization Validation             │")
                logging.info(f"{LOG_PREFIX} [Test] └─────────────────────────────────────────────────────────┘")
                ...
                if model is None:
                    ...
                    # Check 1: Has executor (workers spawned)
                    has_executor_attr = hasattr(model, 'executor') and model.executor is not None
                    logging.info(f"{LOG_PREFIX} [Test]   {'✅' if has_executor_attr else '❌'} [1/4] Has executor (workers spawned)")
                    if has_executor_attr: checks_passed += 1
                    
                    if not has_executor_attr:
                        logging.error(f"{LOG_PREFIX} [Test]   ❌ No executor - workers not spawned")
                        results.append(f"❌ Phase 2.6: No executor ({checks_passed}/{checks_total})")
                    else:
                        test_executor = model.executor
                        
                        # Check 2: Workers already have model loaded (EAGER)
                        try:
                            result = test_executor.execute_collective("check_model_state", {})
                            
                            has_model = result.get("has_model", False)
                            param_count = result.get("param_count", 0)
                            
                            logging.info(f"{LOG_PREFIX} [Test]   {'✅' if has_model else '❌'} [2/4] Workers have model loaded (eager): {param_count} params")
                            if has_model: checks_passed += 1
                            
                        except Exception as e:
                            logging.error(f"{LOG_PREFIX} [Test]   ❌ [2/4] Worker check failed: {e}")
                            import traceback
                            traceback.print_exc()
                        
                        # Check 3: VRAM already reduced (eager init complete)
                        try:
                            vram_result = test_executor.execute_collective("get_model_size", {})
                            vram_gb = vram_result.get("vram_gb", 0)
                            vram_reduced = vram_gb <= 12.0
                            
                            logging.info(f"{LOG_PREFIX} [Test]   {'✅' if vram_reduced else '❌'} [3/4] VRAM already reduced: {vram_gb:.2f}GB (≤12GB)")
                            if vram_reduced: checks_passed += 1
                            
                        except Exception as e:
                            logging.error(f"{LOG_PREFIX} [Test]   ❌ [3/4] VRAM check failed: {e}")
                        
                        # Check 4: Workers initialized in sd.py (not lazy)
                        try:
                            # If workers have model, they were initialized eagerly
                            eager_init = has_model and param_count > 0
                            
                            logging.info(f"{LOG_PREFIX} [Test]   {'✅' if eager_init else '❌'} [4/4] Eager initialization (not lazy)")
                            if eager_init: checks_passed += 1
                            
                        except Exception as e:
                            logging.error(f"{LOG_PREFIX} [Test]   ❌ [4/4] Eager check failed: {e}")
                        
                        # Summary
                        if checks_passed == checks_total:
                            logging.info(f"{LOG_PREFIX} [Test] ✅ PASS: {checks_passed}/{checks_total}")
                            results.append(f"✅ Phase 2.6: Eager Init ({checks_passed}/{checks_total})")
                        else:
                            logging.error(f"{LOG_PREFIX} [Test] ❌ FAIL: {checks_passed}/{checks_total}")
                            results.append(f"❌ Phase 2.6: Eager Init ({checks_passed}/{checks_total})")
            
            # ═══════════════════════════════════════════════════════════════
            # PHASE 2.7: FSDP2 Sharding (Test 2.7) - EAGER VALIDATION
            # ═══════════════════════════════════════════════════════════════
            if phase2_7_fsdp2_sharding:
                logging.info(f"{LOG_PREFIX} [Test] ┌─────────────────────────────────────────────────────────┐")
                logging.info(f"{LOG_PREFIX} [Test] │ PHASE 2.7: FSDP2 Sharding (Eager Validation)           │")
                logging.info(f"{LOG_PREFIX} [Test] └─────────────────────────────────────────────────────────┘")
                ...
                if model is None:
                    ...
                    # Check 2: Has executor
                    has_executor = hasattr(model, 'executor') and model.executor is not None
                    logging.info(f"{LOG_PREFIX} [Test]   {'✅' if has_executor else '❌'} [2/5] Has executor")
                    if has_executor: checks_passed += 1
                    
                    if has_meta and has_executor:
                        # Check 3: Workers already have FSDP2 model (eager init)
                        try:
                            result = model.executor.execute_collective("check_model_state", {})
                            
                            has_model = result.get("has_model", False)
                            param_count = result.get("param_count", 0)
                            
                            logging.info(f"{LOG_PREFIX} [Test]   {'✅' if has_model else '❌'} [3/5] Workers have FSDP2 model: {param_count} params")
                            if has_model: checks_passed += 1
                            
                        except Exception as e:
                            logging.error(f"{LOG_PREFIX} [Test]   ❌ [3/5] Model check failed: {e}")
                            import traceback
                            traceback.print_exc()
                        
                        # Check 4: VRAM already reduced (≤12GB per worker)
                        try:
                            vram_result = model.executor.execute_collective("get_model_size", {})
                            vram_gb = vram_result.get("vram_gb", 0)
                            vram_target = 12.0  # GB
                            vram_reduced = vram_gb <= vram_target
                            
                            logging.info(f"{LOG_PREFIX} [Test]   {'✅' if vram_reduced else '❌'} [4/5] VRAM ≤ {vram_target}GB: {vram_gb:.2f}GB")
                            if vram_reduced: checks_passed += 1
                            
                        except Exception as e:
                            logging.error(f"{LOG_PREFIX} [Test]   ❌ [4/5] VRAM check failed: {e}")
                        
                        # Check 5: Sharding applied (check for DTensor params)
                        try:
                            result = model.executor.execute_collective(
                            ...
                            logging.info(f"{LOG_PREFIX} [Test]   {'✅' if is_sharded else '❌'} [5/5] FSDP2 sharding applied (DTensor params)")
                            if is_sharded: checks_passed += 1
                            
                        except Exception as e:
                            logging.error(f"{LOG_PREFIX} [Test]   ❌ [5/5] Sharding check failed: {e}")
            ...
            # ═══════════════════════════════════════════════════════════════
            # PHASE 2.10: Meta-Shell Architecture (Test 2.10.1)
            # ═══════════════════════════════════════════════════════════════
            if phase2_10_meta_shell:
                logging.info(f"{LOG_PREFIX} [Test] ┌─────────────────────────────────────────────────────────┐")
                logging.info(f"{LOG_PREFIX} [Test] │ PHASE 2.10.1: Meta-Shell Architecture                  │")
                logging.info(f"{LOG_PREFIX} [Test] └─────────────────────────────────────────────────────────┘")
                
                if model is None:
                    logging.info(f"{LOG_PREFIX} [Test] ⏸️  SKIP: No MODEL input")
                    results.append("⏸️  Phase 2.10.1: Skipped (no MODEL)")
                else:
                    checks_passed = 0
                    checks_total = 5
                    
                    # Check 1: Parent model on meta device
                    try:
                        first_param = next(model.model.parameters())
                        is_meta = first_param.device.type == 'meta'
                        logging.info(f"{LOG_PREFIX} [Test]   {'✅' if is_meta else '❌'} [1/5] Parent model on meta device: {first_param.device}")
                        if is_meta: checks_passed += 1
                    except Exception as e:
                        logging.error(f"{LOG_PREFIX} [Test]   ❌ [1/5] Device check failed: {e}")
                    
                    # Check 2: model_sampling exists (recreated on CPU)
                    has_sampling = hasattr(model.model, 'model_sampling') and model.model.model_sampling is not None
                    if has_sampling:
                        try:
                            sampling_device = model.model.model_sampling.sigmas.device
                            is_cpu = sampling_device.type == 'cpu'
                            logging.info(f"{LOG_PREFIX} [Test]   {'✅' if is_cpu else '❌'} [2/5] model_sampling on CPU: {sampling_device}")
                            if is_cpu: checks_passed += 1
                        except Exception as e:
                            logging.error(f"{LOG_PREFIX} [Test]   ❌ [2/5] model_sampling check failed: {e}")
                    else:
                        logging.error(f"{LOG_PREFIX} [Test]   ❌ [2/5] No model_sampling attribute")
                    
                    # Check 3: meta_model same as model (not deepcopy in Phase 2.10)
                    has_meta = hasattr(model, 'meta_model') and model.meta_model is not None
                    if has_meta:
                        same_object = model.meta_model is model.model
                        logging.info(f"{LOG_PREFIX} [Test]   {'✅' if same_object else '❌'} [3/5] meta_model is model (same object): {same_object}")
                        if same_object: checks_passed += 1
                    else:
                        logging.error(f"{LOG_PREFIX} [Test]   ❌ [3/5] No meta_model attribute")
                    
                    # Check 4: Parent VRAM near 0 (meta device uses no memory)
                    if torch.cuda.is_available():
                        try:
                            # Get parent process VRAM (device 0)
                            parent_vram = torch.cuda.memory_allocated(0) / (1024**3)
                            near_zero = parent_vram < 1.0  # Less than 1GB
                            logging.info(f"{LOG_PREFIX} [Test]   {'✅' if near_zero else '❌'} [4/5] Parent VRAM near 0: {parent_vram:.2f}GB")
                            if near_zero: checks_passed += 1
                        except Exception as e:
                            logging.error(f"{LOG_PREFIX} [Test]   ❌ [4/5] VRAM check failed: {e}")
                    else:
                        logging.info(f"{LOG_PREFIX} [Test]   ⏸️  [4/5] CUDA not available, skipping VRAM check")
                        checks_total -= 1
                    
                    # Check 5: Workers have actual model (not meta)
                    if hasattr(model, 'executor') and model.executor is not None:
                        try:
                            result = model.executor.execute_collective("check_model_state", {})
                            has_worker_model = result.get("has_model", False)
                            param_count = result.get("param_count", 0)
                            
                            worker_has_real = has_worker_model and param_count > 0
                            logging.info(f"{LOG_PREFIX} [Test]   {'✅' if worker_has_real else '❌'} [5/5] Workers have real model: {param_count} params")
                            if worker_has_real: checks_passed += 1
                        except Exception as e:
                            logging.error(f"{LOG_PREFIX} [Test]   ❌ [5/5] Worker check failed: {e}")
                    else:
                        logging.error(f"{LOG_PREFIX} [Test]   ❌ [5/5] No executor")
                    
                    # Summary
                    if checks_passed == checks_total:
                        logging.info(f"{LOG_PREFIX} [Test] ✅ PASS: {checks_passed}/{checks_total}")
                        results.append(f"✅ Phase 2.10.1: Meta-Shell ({checks_passed}/{checks_total})")
                    else:
                        logging.error(f"{LOG_PREFIX} [Test] ❌ FAIL: {checks_passed}/{checks_total}")
                        results.append(f"❌ Phase 2.10.1: Meta-Shell ({checks_passed}/{checks_total})")
            
            # ═══════════════════════════════════════════════════════════════
            # PHASE 2.10: Unpicklable Attribute Handling (Test 2.10.2)
            # ═══════════════════════════════════════════════════════════════
            if phase2_10_pickling:
                logging.info(f"{LOG_PREFIX} [Test] ┌─────────────────────────────────────────────────────────┐")
                logging.info(f"{LOG_PREFIX} [Test] │ PHASE 2.10.2: Unpicklable Attribute Handling           │")
                logging.info(f"{LOG_PREFIX} [Test] └─────────────────────────────────────────────────────────┘")
                
                if model is None:
                    logging.info(f"{LOG_PREFIX} [Test] ⏸️  SKIP: No MODEL input")
                    results.append("⏸️  Phase 2.10.2: Skipped (no MODEL)")
                else:
                    checks_passed = 0
                    checks_total = 3
                    
                    # Check 1: Parent has model_sampling (restored after pickling)
                    has_sampling = hasattr(model.model, 'model_sampling') and model.model.model_sampling is not None
                    logging.info(f"{LOG_PREFIX} [Test]   {'✅' if has_sampling else '❌'} [1/3] Parent has model_sampling (restored)")
                    if has_sampling: checks_passed += 1
                    
                    # Check 2: Workers received meta_model successfully (pickling worked)
                    if hasattr(model, 'executor') and model.executor is not None:
                        try:
                            result = model.executor.execute_collective("check_model_state", {})
                            has_model = result.get("has_model", False)
                            
                            logging.info(f"{LOG_PREFIX} [Test]   {'✅' if has_model else '❌'} [2/3] Workers received meta_model (pickling worked)")
                            if has_model: checks_passed += 1
                        except Exception as e:
                            logging.error(f"{LOG_PREFIX} [Test]   ❌ [2/3] Worker check failed: {e}")
                    else:
                        logging.error(f"{LOG_PREFIX} [Test]   ❌ [2/3] No executor")
                    
                    # Check 3: Parent model_sampling still accessible
                    if has_sampling:
                        try:
                            can_access = model.model.model_sampling.sigmas is not None
                            logging.info(f"{LOG_PREFIX} [Test]   {'✅' if can_access else '❌'} [3/3] Parent model_sampling accessible")
                            if can_access: checks_passed += 1
                        except Exception as e:
                            logging.error(f"{LOG_PREFIX} [Test]   ❌ [3/3] Access check failed: {e}")
                    else:
                        logging.error(f"{LOG_PREFIX} [Test]   ❌ [3/3] No model_sampling to access")
                    
                    # Summary
                    if checks_passed == checks_total:
                        logging.info(f"{LOG_PREFIX} [Test] ✅ PASS: {checks_passed}/{checks_total}")
                        results.append(f"✅ Phase 2.10.2: Pickling ({checks_passed}/{checks_total})")
                    else:
                        logging.error(f"{LOG_PREFIX} [Test] ❌ FAIL: {checks_passed}/{checks_total}")
                        results.append(f"❌ Phase 2.10.2: Pickling ({checks_passed}/{checks_total})")
            
            # ═══════════════════════════════════════════════════════════════
            # PHASE 2.10: Worker Initialization Validation (Test 2.10.3)
            # ═══════════════════════════════════════════════════════════════
            if phase2_10_worker_init:
                logging.info(f"{LOG_PREFIX} [Test] ┌─────────────────────────────────────────────────────────┐")
                logging.info(f"{LOG_PREFIX} [Test] │ PHASE 2.10.3: Worker Initialization Validation         │")
                logging.info(f"{LOG_PREFIX} [Test] └─────────────────────────────────────────────────────────┘")
                
                if model is None:
                    logging.info(f"{LOG_PREFIX} [Test] ⏸️  SKIP: No MODEL input")
                    results.append("⏸️  Phase 2.10.3: Skipped (no MODEL)")
                else:
                    checks_passed = 0
                    checks_total = 6
                    
                    if not (hasattr(model, 'executor') and model.executor is not None):
                        logging.error(f"{LOG_PREFIX} [Test]   ❌ No executor")
                        results.append(f"❌ Phase 2.10.3: No executor (0/{checks_total})")
                    else:
                        # Check 1: Workers have model loaded
                        try:
                            result = model.executor.execute_collective("check_model_state", {})
                            has_model = result.get("has_model", False)
                            param_count = result.get("param_count", 0)
                            
                            logging.info(f"{LOG_PREFIX} [Test]   {'✅' if has_model else '❌'} [1/6] Workers have model: {param_count} params")
                            if has_model and param_count > 0: checks_passed += 1
                        except Exception as e:
                            logging.error(f"{LOG_PREFIX} [Test]   ❌ [1/6] Model check failed: {e}")
                        
                        # Check 2: VRAM reduced (≤12GB per GPU)
                        try:
                            vram_result = model.executor.execute_collective("get_model_size", {})
                            vram_gb = vram_result.get("vram_gb", 0)
                            vram_ok = vram_gb <= 12.0
                            
                            logging.info(f"{LOG_PREFIX} [Test]   {'✅' if vram_ok else '❌'} [2/6] Worker VRAM: {vram_gb:.2f}GB (≤12GB)")
                            if vram_ok: checks_passed += 1
                        except Exception as e:
                            logging.error(f"{LOG_PREFIX} [Test]   ❌ [2/6] VRAM check failed: {e}")
                        
                        # Check 3: Sharded params distributed
                        try:
                            result = model.executor.execute_collective(
                                "check_param_sharding",
                                {"param_name": "diffusion_model.double_blocks.0.img_attn.qkv.weight"}
                            ...
                            logging.info(f"{LOG_PREFIX} [Test]   {'✅' if is_sharded else '❌'} [3/6] double_blocks.0 sharded")
                            if is_sharded: checks_passed += 1
                        except Exception as e:
                            logging.error(f"{LOG_PREFIX} [Test]   ❌ [3/6] Sharding check failed: {e}")
                        
                        # Check 4: Ignored params replicated
                        try:
                            result = model.executor.execute_collective(
                                "check_param_sharding",
                                {"param_name": "diffusion_model.img_in.weight"}
                            ...
                            logging.info(f"{LOG_PREFIX} [Test]   {'✅' if is_replicated else '❌'} [4/6] img_in.weight replicated")
                            if is_replicated: checks_passed += 1
                        except Exception as e:
                            logging.error(f"{LOG_PREFIX} [Test]   ❌ [4/6] Replication check failed: {e}")
                        
                        # Check 5: VRAM breakdown correct
                        try:
                            result = model.executor.execute_collective("get_vram_breakdown", {})
                            sharded_vram = result.get("sharded_vram_gb", 0)
                            replicated_vram = result.get("replicated_vram_gb", 0)
                            breakdown_ok = sharded_vram > 8 and replicated_vram < 3
                            
                            logging.info(f"{LOG_PREFIX} [Test]   {'✅' if breakdown_ok else '❌'} [5/6] VRAM: {sharded_vram:.2f}GB sharded, {replicated_vram:.2f}GB replicated")
                            if breakdown_ok: checks_passed += 1
                        except Exception as e:
                            logging.error(f"{LOG_PREFIX} [Test]   ❌ [5/6] Breakdown check failed: {e}")
                        
                        # Check 6: Sharding strategy correct
                        try:
                            result = model.executor.execute_collective("validate_sharding_strategy", {})
                            double_wrapped = result.get("double_blocks_wrapped", 0)
                            single_wrapped = result.get("single_blocks_wrapped", 0)
                            strategy_ok = double_wrapped == 19 and single_wrapped == 38
                            
                            logging.info(f"{LOG_PREFIX} [Test]   {'✅' if strategy_ok else '❌'} [6/6] Strategy: {double_wrapped} double + {single_wrapped} single")
                            if strategy_ok: checks_passed += 1
                        except Exception as e:
                            logging.error(f"{LOG_PREFIX} [Test]   ❌ [6/6] Strategy check failed: {e}")
                        
                        # Summary
                        if checks_passed == checks_total:
                            logging.info(f"{LOG_PREFIX} [Test] ✅ PASS: {checks_passed}/{checks_total}")
                            results.append(f"✅ Phase 2.10.3: Worker Init ({checks_passed}/{checks_total})")
                        else:
                            logging.error(f"{LOG_PREFIX} [Test] ❌ FAIL: {checks_passed}/{checks_total}")
                            results.append(f"❌ Phase 2.10.3: Worker Init ({checks_passed}/{checks_total})")
            ...
            if phase2_8_forward_pass:
                ...
                if model is None:
                    ...
                    # Test 3: VRAM stable
                    try:
                        vram_result = model.executor.execute_collective(
                            "get_model_size",
                            {}
                        )
            ...
            if executor and not (model is not None and hasattr(model, 'executor') and model.executor is executor):
            ...
            try:
                if 'executor' in locals() and executor and not (model is not None and hasattr(model, 'executor') and model.executor is executor):
                    executor.shutdown()
            except:
                pass
