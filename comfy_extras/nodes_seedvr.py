...
class SeedVR2InputProcessing(io.ComfyNode):
    ...
    def execute(cls, images, vae, resolution, spatial_tile_size, spatial_overlap, temporal_tile_size, temporal_overlap, enable_tiling):
        ...
        if enable_tiling:
            ...
            try:
                if multigpu_clones:
                    for dev, clone in multigpu_clones.items():
                        comfy.model_management.free_memory(clone.memory_used_encode(images_bcthw.shape, clone.vae_dtype), dev)
                        clone.first_stage_model.to(dev)
                        clone.first_stage_model.device = dev
                ...
                for dev, clone in dict(multigpu_clones or {}).items():
                    if dev in clone_previous_args:
                        clone.first_stage_model.tiled_args = clone_previous_args[dev]
                    clone.first_stage_model.to("cpu")
        else:
            vae_model.img_dims = [o_h, o_w]
            vae_model.original_image_video = original_image_video
