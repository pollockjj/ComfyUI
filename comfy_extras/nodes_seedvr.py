...
class SeedVR2Conditioning(io.ComfyNode):
    ...
    def execute(cls, vae_conditioning, model, latent_noise_scale) -> io.NodeOutput:
        ...
        if vae_conditioning.shape[1] != _SEEDVR2_LATENT_CHANNELS:
            if vae_conditioning.shape[-1] == _SEEDVR2_LATENT_CHANNELS:
                raise ValueError(
            ...
            raise ValueError(
                "SeedVR2Conditioning expects SeedVR2 VAE latents in Comfy "
                f"channel-first layout with {_SEEDVR2_LATENT_CHANNELS} latent "
                f"channels; got shape {tuple(vae_conditioning.shape)}."
            )
