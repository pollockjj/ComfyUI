...
class DiffusionGemmaExperts(nn.Module):
    ...
    def _forward_grouped_int8_convrot(self, hidden_states, top_k_index, top_k_weights):
        ...
        weighted_routes = comfy.quant_ops.ck.restore_weighted_int8_moe_routes(
            y, pair_order, top_k_weights.reshape(-1)
        )
        return weighted_routes.view(N, K, H).sum(dim=1).to(hidden_states.dtype)
