...
class AttentionBackend(ABC):
    ...
    def needs_sequence_parallel_wrapper() -> bool:
        """Return True if backend requires DistributedAttention to handle all-to-all.
        
        Returns False for backends like xFuser that handle communication internally.
        """
        return True
    ...
    def get_world_size() -> int:
        """Return the parallel world size for this backend's process group."""
    
    @staticmethod
    @abstractmethod
    def get_rank() -> int:
        """Return the current rank in this backend's process group."""
    
    @staticmethod
    @abstractmethod
    def all_to_all_4d(tensor: Tensor, scatter_dim: int, gather_dim: int) -> Tensor:
        """Backend-specific all-to-all collective for 4D tensors.
        
        Args:
            tensor: Input tensor [batch, seq, heads, dim]
            scatter_dim: Dimension to scatter across ranks
            gather_dim: Dimension to gather from ranks
            
        Returns:
            Redistributed tensor with dimensions transformed
        """
    
    @staticmethod
    @abstractmethod
    def all_gather_nd(tensor: Tensor, dim: int) -> Tensor:
        """Backend-specific all-gather collective for N-D tensors.
        ...
        Returns:
            Gathered tensor with dimension increased by world_size
        """
