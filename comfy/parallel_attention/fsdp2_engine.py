...
def apply_fsdp2_sharding(
    ...
    Example:
        ...
        model = apply_fsdp2_sharding(meta_model, config, checkpoint_path)
    """
    ...
    logging.info(
        f"{LOG_PREFIX} [FSDP2Engine] Ignoring {len(ignored_params)} params "
        f"(patterns: {', '.join(config.ignored_param_patterns)})"
    )
    ...
    for block_config in config.blocks:
        ...
        logging.info(
            f"{LOG_PREFIX} [FSDP2Engine] Sharding {len(block_list)} blocks "
            f"at {block_config.module_path}..."
        )
