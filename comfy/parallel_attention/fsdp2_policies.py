...
def wan_fsdp2_policy() -> ShardingConfig:
    ...
    Wan architecture:
    - blocks: 30 transformer blocks (Wan2.1/2.2)
    - Other components: patch_embedding, head, time_embedding, text_embedding, etc.
    
    Sharding strategy (matches Raylight EXACTLY):
    - Shard: blocks.* (all params where name.startswith("blocks."))
    - Ignore: Everything else (patch_embedding, head, time_embedding, etc.)
    
    Raylight policy: ignored_params = params where NOT name.startswith("blocks.")
...
def qwen_image_fsdp2_policy() -> ShardingConfig:
    ...
    return ShardingConfig(
        ...
        blocks=[
            BlockConfig(
                module_path="transformer_blocks",
                block_count=60,
                shard_each=True
            ),
        ],
