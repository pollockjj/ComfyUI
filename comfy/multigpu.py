...
def create_vae_multigpu_deepclones(vae, max_gpus: int):
    ...
    # The result is a patcher whose model has zero inherited aimdo state — concurrent
    # worker threads on the extra device access weights through the target-device buffer
    # rather than through stale source-device aimdo storage.
    for device in limit_extra_devices:
        if device in clones:
            continue
        cloned_patcher = vae.patcher.deepclone_multigpu(new_load_device=device)
        # Build a fresh VAE wrapper around the clone's first_stage_model. We shallow-copy
        # the source VAE (carries process_input, vae_dtype, downscale_ratio, etc.) and
        # swap in the deepcloned model + patcher.
