...
class _DynamicVRAMExecutionLease:
    _MODULE_ATTR = "_dynamic_vram_execution_lease"

    def __init__(self, modules, page_budgets):
        self.modules = modules
        self.page_budgets = page_budgets
        self.retained_pages = {key: set() for key in page_budgets}
        self.retained_allocations = {}
        self.pinned_allocations = []

    @classmethod
    def capture(cls, root_module, reserve_bytes):
        if reserve_bytes < 0:
            raise ValueError("DynamicVRAM execution reserve must be non-negative")
        ...
        for module in root_module.modules():
            ...
            if allocation is None:
                continue
            if getattr(module, cls._MODULE_ATTR, None) is not None:
                raise RuntimeError("DynamicVRAM module already has an execution lease")
            modules.append(module)
            vbars[id(allocation[0])] = allocation[0]

        page_budgets = {
            key: max(0, (vbar.loaded_size() - reserve_bytes) // _DYNAMIC_VRAM_PAGE_SIZE)
            for key, vbar in vbars.items()
        }
        if not modules or not any(page_budgets.values()):
            return None

        lease = cls(tuple(modules), page_budgets)
        try:
            for module in modules:
                setattr(module, cls._MODULE_ATTR, lease)
        except BaseException:
            lease.release()
            raise
        return lease

    def retain(self, module):
        if getattr(module, self._MODULE_ATTR, None) is not self:
            raise RuntimeError("DynamicVRAM execution lease does not own module")

        allocation = getattr(module, "_v", None)
        if allocation is None:
            return False
        vbar, address, size = allocation
        allocation_key = (id(vbar), address, size)
        if allocation_key in self.retained_allocations:
            return True
        ...
        retained_pages = self.retained_pages[id(vbar)]
        new_pages = pages - retained_pages
        if len(retained_pages) + len(new_pages) > self.page_budgets[id(vbar)]:
            return False
        ...
        if not comfy_aimdo.model_vbar.vbar_signature_compare(current_signature, signature):
            comfy_aimdo.model_vbar.vbar_unpin(allocation)
            raise RuntimeError("DynamicVRAM execution allocation changed while pinned")

        retained_pages.update(new_pages)
        self.retained_allocations[allocation_key] = allocation
        self.pinned_allocations.append(allocation)
        return True

    @property
    def retained_bytes(self):
        return sum(len(pages) for pages in self.retained_pages.values()) * _DYNAMIC_VRAM_PAGE_SIZE
    ...
    def release(self):
        ...
        self.modules = ()
        for module in modules:
            if getattr(module, self._MODULE_ATTR, None) is self:
                delattr(module, self._MODULE_ATTR)
        pinned_allocations = self.pinned_allocations
        self.pinned_allocations = []
        for allocation in reversed(pinned_allocations):
            comfy_aimdo.model_vbar.vbar_unpin(allocation)
...
def acquire_dynamic_vram_execution_lease(root_module, reserve_bytes):
    if not comfy.memory_management.aimdo_enabled:
        return None
    return _DynamicVRAMExecutionLease.capture(root_module, reserve_bytes)
