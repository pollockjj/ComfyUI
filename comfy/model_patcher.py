...
class ModelPatcher:
    ...
    def __del__(self):
        ...
        model_name = self.model.__class__.__name__ if hasattr(self, 'model') and self.model else "UNKNOWN"
        pid = os.getpid()
        refcount = sys.getrefcount(self) if self else -1
        proxy_id = getattr(self, '_proxy_instance_id', 'NO_PROXY_ID')
        # Get caller info
        stack = ''.join(traceback.format_stack()[-5:-1])
        print(f"][ MP:__del__ | pid={pid} | model={model_name} | id={id(self)} | proxy_id={proxy_id} | refs={refcount}")
        print(f"][ MP:__del__ STACK:\n{stack}")
        self.unpin_all_weights()
