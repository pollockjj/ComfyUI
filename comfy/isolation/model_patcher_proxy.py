...
class ModelPatcherProxy(BaseProxy[ModelPatcherRegistry]):
    ...
    def clone(self) -> ModelPatcherProxy:
        ...
        return ModelPatcherProxy(new_id, self._registry, manage_lifecycle=True)
    ...
    def load_lora(self, lora_path: str, strength_model: float, clip: Optional[Any] = None, strength_clip: float = 1.0) -> tuple:
        ...
        if result.get("model_id"):
            new_model = ModelPatcherProxy(result["model_id"], self._registry, manage_lifecycle=True)
        new_clip = None
        if result.get("clip_id"):
            from comfy.isolation.clip_proxy import CLIPProxy
            new_clip = CLIPProxy(result["clip_id"], manage_lifecycle=True)
    ...
    def get_additional_models(self) -> List[ModelPatcherProxy]:
        ...
        return [ModelPatcherProxy(mid, self._registry, manage_lifecycle=True) for mid in ids]
...
class _InnerModelProxy:
    ...
    def __getattr__(self, name: str) -> Any:
        ...
        if name == 'current_patcher':
             return ModelPatcherProxy(self._parent._instance_id, self._parent._registry, manage_lifecycle=True)
