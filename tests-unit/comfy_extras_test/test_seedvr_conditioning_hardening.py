...
def test_seedvr2_conditioning_rejects_non_seedvr2_latent_channels():
    nodes_seedvr, restore = _import_nodes_seedvr_isolated()
    try:
        diffusion_model = _DiffusionModel()
        patcher = _ModelPatcher(diffusion_model)
        vae_conditioning = {"samples": torch.zeros((1, 4, 1, 1, 1))}
        ...
        assert "16 latent channels" in message
        assert "(1, 4, 1, 1, 1)" in message
    finally:
        restore()
