...
# ---------------------------------------------------------------------------
# CustomRMSNorm dtype tests
# ---------------------------------------------------------------------------

def test_custom_rms_norm_preserves_input_dtype_after_fp32_math():
    norm = CustomRMSNorm(4, dtype=torch.float32)
...
def test_na_7b_rope_keeps_freqs_fp32_for_low_precision_inputs():
    rope = NaRotaryEmbedding3d(dim=_DIM)
    vid_q, vid_k, vid_shape, _, _, _, cache = _make_inputs(dtype=torch.bfloat16)
    freqs = rope.get_freqs(vid_shape)

    from einops import rearrange

    q_3d = rearrange(vid_q, "L h d -> h L d")
    expected = seedvr_model.apply_lucidrains_rotary_emb(freqs, q_3d.float()).to(q_3d.dtype)
    degraded = seedvr_model.apply_lucidrains_rotary_emb(
        freqs.to(dtype=q_3d.dtype),
        q_3d.float(),
    ).to(q_3d.dtype)

    q_out, _ = rope(vid_q, vid_k, vid_shape, cache)
    q_out = rearrange(q_out, "L h d -> h L d")

    torch.testing.assert_close(q_out, expected, rtol=0, atol=0)
    assert not torch.equal(expected, degraded)
