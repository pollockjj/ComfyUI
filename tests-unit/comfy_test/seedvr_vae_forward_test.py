...
from comfy.ldm.seedvr.vae import VideoAutoencoderKL, VideoAutoencoderKLWrapper  # noqa: E402
...
_WRAPPER_INPUT_SHAPE = (1, 3, 5, 16, 16)
_WRAPPER_POSTERIOR_SHAPE = (1, 16, 1, 2, 2)
_WRAPPER_DECODE_OUT_SHAPE = (1, 3, 5, 16, 16)
...
def test_wrapper_forward_returns_tensor_triple(monkeypatch):
    ...
    wrapper.original_image_video = torch.zeros(*_WRAPPER_INPUT_SHAPE)
    wrapper.img_dims = (16, 16)
    wrapper.freeze_encoder = True

    posterior = torch.full(_WRAPPER_POSTERIOR_SHAPE, 7.0)
    decode_out = torch.full(_WRAPPER_DECODE_OUT_SHAPE, 13.0)
    ...
    x = torch.zeros(*_WRAPPER_INPUT_SHAPE)
