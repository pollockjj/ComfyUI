    ...
    try:
        ...
        try:
            with patch.object(vae_mod.F, "group_norm", side_effect=_group_norm_spy):
                out_tensor = causal_norm_wrapper(gn, x)
        finally:
            handle.remove()
        ...
        return out_tensor, full_calls, chunked_calls
    finally:
        set_norm_limit(None)


def test_seedvr_groupnorm_default_limit_uses_full_groupnorm_path():
    out_tensor, full_calls, chunked_calls = _run_with_limit(None)
...
def test_seedvr_groupnorm_low_limit_uses_chunked_groupnorm_path():
    out_tensor, full_calls, chunked_calls = _run_with_limit(1e-9)
