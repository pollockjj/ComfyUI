...
The expected behavior converts every non-SeedVR2 VAE carrying diffusers keys;
metadata must not bypass conversion because the downstream VAE loader expects
ComfyUI key naming.

Four cells exercise the reachable metadata shapes — missing key, ``None``,
explicit value, empty dict — and halt the constructor at the first post-guard
call (``model_management.is_amd``).
``_make_standin`` borrows ``__init__`` onto a bare class, mirroring
``seedvr_model_test.py::_make_standin``. ``_exercise_guard`` single-sources the
patched-constructor harness so the cells stay synchronised.
"""
...
def test_diffusers_guard_invokes_convert_when_metadata_is_none():
    """AC2: metadata is ``None`` → convert is invoked."""
...
def test_diffusers_guard_invokes_convert_when_metadata_pins_keep_false():
    """AC3: metadata pins a value → convert is invoked."""
...
def test_diffusers_guard_invokes_convert_when_metadata_is_empty_dict():
    """AC4: metadata is ``{}`` → convert is invoked."""
