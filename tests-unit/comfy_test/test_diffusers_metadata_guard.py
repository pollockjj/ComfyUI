...
raising ``KeyError`` when ``metadata`` was non-``None`` but lacked that key.
The expected behavior uses ``metadata.get("keep_diffusers_format") != "true"``:
a missing key flows through to ``convert_vae_state_dict``; the explicit
``"true"`` value bypasses it.
