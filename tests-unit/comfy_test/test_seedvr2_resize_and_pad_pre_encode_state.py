...
def test_resize_and_pad_returns_input_pixels_original_image_and_shorter_edge():
    images = torch.zeros(1, 3, 16, 16, 3)

    output = nodes_seedvr.SeedVR2ResizeAndPad.execute(images, 120)

    input_pixels, original_image, shorter_edge = output.result
...
def test_resize_and_pad_schema_and_execute_signature_are_preprocess_only():
    ...
    assert [item.id for item in schema.outputs] == ["input_pixels", "original_image", "shorter_edge"]
