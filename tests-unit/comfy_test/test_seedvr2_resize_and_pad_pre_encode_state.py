...
def test_resize_and_pad_returns_input_pixels_original_image_and_upscaled_shorter_edge():
    ...
    output = nodes_seedvr.SeedVR2ResizeAndPad.execute(images, 128, 1.0)
    ...
    assert upscaled_shorter_edge == 128


def test_resize_and_pad_multiplier_resolves_upscaled_shorter_edge():
    images = torch.zeros(1, 1, 16, 16, 3)

    output = nodes_seedvr.SeedVR2ResizeAndPad.execute(images, 128, 2.0)

    input_pixels, original_image, upscaled_shorter_edge = output.result
    assert tuple(input_pixels.shape) == (1, 1, 256, 256, 3)
    assert original_image is images
    assert upscaled_shorter_edge == 256
...
def test_resize_and_pad_rejects_non_positive_multiplier():
    ...
    try:
        nodes_seedvr.SeedVR2ResizeAndPad.execute(images, 128, 0.0)
...
def test_resize_and_pad_rejects_non_divisible_resolved_shorter_edge():
    images = torch.zeros(1, 1, 16, 16, 3)

    try:
        nodes_seedvr.SeedVR2ResizeAndPad.execute(images, 120, 1.0)
    except ValueError as e:
        assert "divisible by 16" in str(e)
        assert "got 120" in str(e)
    else:
        raise AssertionError("non-divisible shorter edge was not rejected")
...
def test_resize_and_pad_schema_and_execute_signature_are_preprocess_only():
    ...
    assert schema.inputs[1].min == 128
    assert schema.inputs[1].step == 16
